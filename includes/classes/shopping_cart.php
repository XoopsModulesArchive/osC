...
// ############ Added CCGV Contribution ##########
          $this->total_virtual += ($products_tax * $qty) * $no_count;// ICW CREDIT CLASS;
          $this->weight_virtual += ($qty * $products_weight) * $no_count;// ICW CREDIT CLASS;
// ############ End Added CCGV Contribution ##########
/*
This function is designed to show tax included when purchasing an item. However, getdisplaytax() is not defined
nor can it be by calling the function from /includes/customer_group.php
        if ($customer_group->getdisplaytax() == '1' ) {$this->total += tep_add_tax($products_price, $products_tax) * $qty;} else { */
...
// attributes price
    ...
    function get_products() {
      ...
      reset($this->contents);
      while (list($products_id, ) = each($this->contents)) {
        $products_query = $mydb->query("select p.products_id, pd.products_name, p.products_model, p.products_image, p.products_price, p.products_cost, p.products_weight, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$products_id . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
