...
*/
	...
	$sql="select p.products_id, p.products_image,pd.products_name,p.products_tax_class_id, if(s.status, s.specials_new_products_price, p.products_price) as products_price from " . TABLE_PRODUCTS . " p left join " . TABLE_SPECIALS . " s on p2c.products_id = s.products_id left join ".TABLE_PRODUCTS_DESCRIPTION." pd on pd.products_id=p.products_id  where p.products_status = '1' and pd.language_id=".$languages_id." order by p.products_date_added desc limit 5";
