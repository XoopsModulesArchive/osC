	...
	function iBrowser2_open() {		
		...
		if (ib.isMSIE) { // IE
			...
			if (rArgs) { // returning from iBrowser (IE) and calling callback function				
				iBrowser2_callback('','',rArgs);
			}
		} else if (ib.isGecko) { // Gecko 
			var wnd = window.open(ib.baseURL + '?editor=' + ib.editor + '&callback=iBrowser2_callback', 'ibrowser', 'status=no, modal=yes, width=625, height=530');
			wnd.dialogArguments = wArgs;
		}
	}
	//-------------------------------------------------------------------------
	// iBrowser callback
	function iBrowser2_callback(editor, sender, iArgs) {		
		if (iArgs) { // IE			
			var rArgs = iArgs;
		} else { // Gecko
		...
		ib.oEditor.contentWindow.document.write('');
		ib.oEditor.contentWindow.document.close();
