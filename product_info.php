...
$add_pinfo_tax='';
  if ($customer_group->getdisplaytax() == '1' ) {
      $add_pinfo_tax= tep_get_tax_rate($product_info['products_tax_class_id']);
	}

    if ($new_price = tep_get_products_special_price($product_info['products_id'])) {
      $products_price = '<s>' . $currencies->display_price($product_info['products_price'], $add_pinfo_tax) . '</s> <span class="productSpecialPrice">' . $currencies->display_price($new_price, $add_pinfo_tax) . '</span>';
    } else {
      $products_price ="". $currencies->display_price($product_info['products_price'], $add_pinfo_tax)."";
...
if (tep_not_null($product_info['products_model'])) {
	...
	if ($customer_group->getdisplaytax() == '1' ) {
      ...
      $add_tax= tep_get_tax_rate($product_info['products_tax_class_id']);
	}
...
*/
    ...
    if ($products_attributes['total'] > 0) {
	 ...
	 $ppreis=$product_info['products_price']+($product_info['products_price']*$add_pinfo_tax);
...
//	  $jscript.='id[' . $products_options_name['products_options_id'] . '] = new array();';
        ...
        while ($products_options = tep_db_fetch_array($products_options_query)) {
		  ...
		  if($products_options['price_prefix'] =='-' ) {
		   ...
		   $pprice=$diff_price+($diff_price*$add_pinfo_tax/100);
